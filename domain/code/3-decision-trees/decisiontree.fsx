...
// ----------------------------------------------------------------------------
type FeatureMethod<'T> = 
  | Categorical of ('T -> string)
  | Numerical of ('T -> float)

type Feature<'T> = Feature of string * FeatureMethod<'T>
...
// TASK #1: Run a decision tree on sample input. To do this, we 
// need to pattern match on the decision tree node. If it is 
// `DecisionResult`, we return result; if it is `DecisionChoice`,
// we find the feature and run it on the sample row.

let getFeatureValue name input = 
  Titanic.features 
  |> Seq.choose (fun (Feature(n, f)) ->
      if n = name then Some(f input)
      else None)

let runTree tree node = 
  let rec run t n = 
    match t with
    | DecisionChoice c ->
         let value = Seq.exactlyOne (getFeatureValue c.Feature n)
         if value > c.Threshold then
            run c.MoreThan n
         else
            run c.LessThan n
    | DecisionResult r -> r
...
let input = Titanic.data |> Seq.head
let expected = string input.Survived
let actual = runTree Titanic.tree input

// `getFeatureValue` is a helper function that gets a value of a 
// specified feature - you will need this to implement `runTree`.
getFeatureValue "Age" input
getFeatureValue "Siblings" input
 


// TASK #2: Evaluate how good our decision trees are - calculate the 
// ratio of <correctly_classified>/<total> for all items from our data.

// (...)
Seq.map 
  (fun (n : Titanic.TitanicRow) -> 
    (n.Survived, runTree Titanic.tree n)) Titanic.data
  |> Seq.countBy 
    (fun (a, b) -> match a, b with 
                    | true, "True" | false, "False" -> "Correct match" 
                    | _ -> "Incorrect match") 
  |> Seq.toList

// TASK #3: Change feature so that the result is either "Numerical of float"
// or "Categorical of string". For numerical features (already implemented),
// we assume that the feature returns a range of values. For categorical 
// features, we assume that it returns one of several values. When building
// a tree, we want to generate conditions of the form "Categorical = <some-value>"
// To do that, you'll need to iterate over possible values and find which
// one maximizes the entropy growth.
